-- Collect seller KRA PIN during Compliance onboarding.
...
begin
  ...
  v_kra_pin := nullif(upper(regexp_replace(trim(coalesce(p_kra_pin, '')), '\s+', '', 'g')), '');
  if v_kra_pin is not null and v_kra_pin !~ '^[A-Z][0-9]{9}[A-Z]$' then
    raise exception 'Invalid KRA PIN — use format A123456789Z';
  end if;
  if v_plan = 'COMPLIANCE' and v_kra_pin is null then
    raise exception 'KRA PIN is required for Compliance (ETR)';
  end if;
  ...
  insert into public.tenants (
    ...
    location_lat, location_lng, address_text, kra_pin
  ) values (
    trim(v_shop_name),
    trim(v_shop_name),
    v_email,
  ...
  insert into public.shops (
    tenant_id, name, category, phone, address_text, location_lat, location_lng, is_default, kra_pin
  ) values (
    v_tenant_id, trim(v_shop_name), p_category, v_phone,
    p_address_text, p_location_lat, p_location_lng, true, v_kra_pin
  )
...
revoke all on function public.complete_vendor_onboarding(
  ...
  text, text, numeric, numeric, text, text, text, jsonb, text
) from public, anon;
...
-- Keep older 12-arg signature callable (PostgREST may still resolve it).
-- Dropping it avoids ambiguity; clients should pass p_kra_pin (null for Standard).
